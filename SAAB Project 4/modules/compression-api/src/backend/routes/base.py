...
async def create_file(request: Request, file_request: FileCreateRequest):
    """Create a new file record."""
    from ..couchbase.collections.files import FilesCollection, FilesDoc, FileState
    ...
    return FileResponse(
        ...
        archive_id=file_doc.archive_id,
...
async def get_file(request: Request, file_id: UUID):
    """Get a file by ID."""
...
async def list_files(request: Request, limit: int = 50, offset: int = 0):
    ...
    from ..couchbase.collections.files import FilesCollection, ListParams
    ...
    file_docs = await files_collection.list(params)
...
async def create_archive(request: Request, archive_request: ArchiveCreateRequest):
    """Create a new archive and trigger compression workflow."""
    from ..couchbase.collections.archives import ArchivesCollection, ArchivesDoc, ArchiveState
    from ..workflows.compression_workflows import FileCompressionWorkflow
    import uuid
    ...
    for file_id in archive_request.file_ids:
        file_doc = await files_collection.get(file_id)
        if not file_doc:
    ...
    archive_doc = ArchivesDoc(
        name=archive_request.name,
    ...
    # Start Temporal workflow for compression
    temporal_client = request.app.state.temporal_client
    workflow_id = f"compress-archive-{archive_doc.id}"
    
    await temporal_client.start_workflow(
        FileCompressionWorkflow.run,
        args=[str(archive_doc.id), [str(fid) for fid in archive_request.file_ids]],
        id=workflow_id,
        task_queue=temporal_client._config.task_queue,
    )
    ...
    return ArchiveResponse(
        ...
        file_ids=archive_doc.file_ids,
        state=archive_doc.state.value,
        size=archive_doc.size,
        error_message=archive_doc.error_message,
        created_at=archive_doc.created_at,
        updated_at=archive_doc.updated_at,
    )

@router.get("/archives/{archive_id}", response_model=ArchiveResponse)
async def get_archive(request: Request, archive_id: UUID):
    ...
    archive_doc = await archives_collection.get(archive_id)
    if not archive_doc:
...
async def list_archives(request: Request, limit: int = 50, offset: int = 0):
    """List all archives with pagination."""
    from ..couchbase.collections.archives import ArchivesCollection, ListParams
    ...
    params = ListParams(limit=limit, offset=offset)
    archive_docs = await archives_collection.list(params)
...
async def download_archive(request: Request, archive_id: UUID):
    """Download an archive's compressed data."""
    from ..couchbase.collections.archives import ArchivesCollection, ArchiveState
    ...
    if archive_doc.state != ArchiveState.IDLE:
        raise HTTPException(
            status_code=400, 
            detail=f"Archive is not ready for download. Current state: {archive_doc.state.value}"
        )
    
    if not archive_doc.compressed_data:
        raise HTTPException(status_code=404, detail="Archive data not found")
    
    # Update state to DOWNLOADING
    archive_doc.state = ArchiveState.DOWNLOADING
    archive_doc.updated_at = datetime.now(timezone.utc)
    await archives_collection.upsert(archive_doc)
    
    # Decode compressed data
    compressed_data = base64.b64decode(archive_doc.compressed_data)
    
    # Update state back to IDLE after download starts
    archive_doc.state = ArchiveState.IDLE
    archive_doc.updated_at = datetime.now(timezone.utc)
    await archives_collection.upsert(archive_doc)
    ...
    return Response(
        ...
        headers={
            "Content-Disposition": f"attachment; filename={archive_doc.name}.zip"
        }
    )
