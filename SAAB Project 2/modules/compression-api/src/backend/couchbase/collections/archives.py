...
class ArchivesDoc(BaseModel):
    ...
    file_ids: List[UUID] = Field(default_factory=list, description="List of file IDs in this archive")
    total_size: int = Field(default=0, description="Total size of all files in bytes")
    created_at: datetime = Field(default_factory=datetime.utcnow)
    completed_at: Optional[datetime] = None
    archive_path: Optional[str] = None  # Path to the compressed archive file
    download_url: Optional[str] = None  # URL for downloading the archive
